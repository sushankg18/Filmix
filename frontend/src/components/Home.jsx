...
const Home = () => {
    ...
    // setAlreadyInFav(authUser)

    const fetchData = async (page) => {
        try {
            const response = await axios.get(`${BaseUrl}/discover/movie?api_key=${ApiKey}&language=en-U&region=IN&with_original_language=hi&sort_by=popularity.desc&page=${page}`)
    ...
    useEffect(() => {
        fetchData(page)
        setAlreadyInFav(authUser?.watchlater)
    }, [page, authUser])
    ...
    return (
        <Box w={'100%'} bgColor={'transparent'} minH={'90vh'} display={'flex'} alignItems={'center'} justifyContent={'center'} padding={'1rem 2rem 3rem 2rem'}>
            <Box gap={'1.3rem'} w={'100%'} display={'flex'} justifyContent={'center'} flexDir={'column'}>
                <Heading alignSelf={'center'} fontSize={'2rem'} color={'#FC4444'}>Popular</Heading>
                <InfiniteScroll
                ...
                >
                    <SimpleGrid pb={'2rem'} columns={[2, 3, 5, 7]} px={['0', '1rem']} spacing={'2rem'}>
                        {
                            movies.map((item, idx) => (
                                <Box w={'100%'} display={'flex'} flexDir={'column'} alignItems={'center'} justifyContent={'center'} >
